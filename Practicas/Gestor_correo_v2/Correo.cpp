...
void correoNuevo(tCorreo & correo, string emisor) {
	...
	correo.almacenado = 0; // A 0 poque el correo aun no se a insertado en ninguna bandeja
...
void cargar(tCorreo & correo, ifstream & fIn) {
	...
	fIn >> correo.id;
	fIn >> correo.fecha >> correo.almacenado >> correo.emisor;
	fIn >> correo.destinatarios.cont;
	for (int i = 0; i < correo.destinatarios.cont; i++) {
		fIn >> correo.destinatarios.destinatario[i];
	}
	// Al hacer este doble getline evitamos que tenga un error al leer dado que leera hasta el primer salto, es decir, no leera la linea que debe
	getline(fIn, aux);
